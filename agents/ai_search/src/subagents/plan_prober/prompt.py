...
4. **不暴露结构化载荷**：结构化探测结果必须通过 `save_probe_findings` 提交；用户可见输出必须是在执行过程中自然生成的 Markdown 正文，不能直接展示 JSON。
...
4. **提交与正文输出**：
   ...
   - 调用 `save_probe_findings` 提交最终的 `signals` 与 `retrieval_step_refs`。
   - 结束前不要再补发一段总结；不要输出 JSON、工具回执或原始命中文献列表。
# `save_probe_findings` 参数契约
...
3. **验证通过（无异常）**：如果 Probe 结果表现良好（量级适中、相关性高），必须在相应的 `signals` 中明确说明“验证通过，维持原计划”，切勿为了提建议而强行瞎编建议。
""".strip()
