...
from agents.ai_search.src.subagents.query_executor.schemas import ExecutionRoundSummary
...
def build_query_executor_agent(storage: object, task_id: str):
    ...
    return create_deep_agent(
        model=default_model(),
        tools=context.build_query_executor_tools() + build_search_tools(storage, task_id),
        system_prompt=QUERY_EXECUTOR_SYSTEM_PROMPT,
        middleware=[build_guard_middleware("query-executor", storage, task_id)],
