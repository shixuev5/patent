...
_UNSET = object()
_PLANNING_STAGE_KINDS = {"search-elements", "plan-prober", "planner"}
_PHASE_STAGE_KINDS = {
    ...
    PHASE_AWAITING_HUMAN_DECISION: "feature-comparer",
}
...
class AiSearchAgentContext:
    ...
    def _stage_descriptor(self, stage_kind: str) -> Dict[str, Any]:
        normalized_stage = str(stage_kind or "").strip()
        run = self.active_run()
        run_id = str(run.get("run_id") or "").strip() if isinstance(run, dict) else ""
        if normalized_stage in _PLANNING_STAGE_KINDS:
            plan_version = self.target_plan_version()
        else:
            plan_version = int(self.active_plan_version() or (run.get("plan_version") if isinstance(run, dict) else 0) or 0)
        todo_id = str(run.get("active_retrieval_todo_id") or "").strip() if isinstance(run, dict) else ""
        batch_id = str(run.get("active_batch_id") or "").strip() if isinstance(run, dict) else ""
        if normalized_stage == "query-executor":
            suffix = todo_id or f"plan-{plan_version or 0}"
        elif normalized_stage in {"coarse-screener", "close-reader"}:
            suffix = batch_id or f"plan-{plan_version or 0}"
        elif normalized_stage == "feature-comparer":
            suffix = batch_id or f"plan-{plan_version or 0}"
        else:
            suffix = f"plan-{plan_version or 0}"
        return {
            "stageKind": normalized_stage,
            "stageInstanceId": f"{normalized_stage}:{suffix}",
            "runId": run_id or None,
            "planVersion": plan_version or None,
            "todoId": todo_id or None,
            "batchId": batch_id or None,
        }

    def default_stage_kind_for_phase(self, phase: str = "") -> str:
        normalized_phase = str(phase or self.current_phase() or "").strip()
        return str(_PHASE_STAGE_KINDS.get(normalized_phase) or "").strip()

    def _list_stage_messages(self, stage_instance_id: str) -> List[Dict[str, Any]]:
        target = str(stage_instance_id or "").strip()
        if not target:
            return []
        matched: List[Dict[str, Any]] = []
        for item in reversed(self.storage.list_ai_search_messages(self.task_id)):
            if str(item.get("kind") or "").strip() != "assistant_stage_message":
                continue
            metadata = item.get("metadata")
            if not isinstance(metadata, dict):
                continue
            if str(metadata.get("stageInstanceId") or "").strip() == target:
                matched.append(item)
        return matched

    def _has_stage_runtime_event(
        self,
        ...
        *,
        ...
        normalized_event = str(runtime_event or "").strip()
        normalized_checkpoint = str(runtime_checkpoint or "").strip()
        if not normalized_event and not normalized_checkpoint:
            return False
        for item in self._list_stage_messages(stage_instance_id):
            metadata = item.get("metadata") if isinstance(item.get("metadata"), dict) else {}
            if normalized_event and str(metadata.get("runtimeEvent") or "").strip() == normalized_event:
                return True
            if normalized_checkpoint:
                checkpoints = metadata.get("runtimeCheckpoints") if isinstance(metadata.get("runtimeCheckpoints"), list) else []
                if normalized_checkpoint in [str(value or "").strip() for value in checkpoints]:
                    return True
                if str(metadata.get("runtimeCheckpoint") or "").strip() == normalized_checkpoint:
                    return True
        return False
    ...
    def _startup_objective(self) -> str:
        search_elements = self.current_search_elements()
        objective = str(search_elements.get("objective") or "").strip()
        if objective:
            return objective
        source_summary = self._source_patent_summary()
        return source_summary.get("invention_title") or source_summary.get("publication_number") or ""

    def _startup_search_element_count(self) -> int:
        search_elements = self.current_search_elements()
        raw_items = search_elements.get("search_elements") if isinstance(search_elements.get("search_elements"), list) else []
        return len([item for item in raw_items if isinstance(item, dict)])

    def _startup_selected_document_count(self) -> int:
        run = self.active_run()
        if not isinstance(run, dict):
            return 0
        return int(run.get("selected_document_count") or 0)

    def _build_startup_stage_log_content(self, stage_kind: str) -> str:
        stage = str(stage_kind or "").strip()
        objective = self._startup_objective()
        objective_clause = f"围绕“{objective}”" if objective else "围绕当前检索目标"
        search_element_count = self._startup_search_element_count()
        source_summary = self._source_patent_summary()
        source_label = source_summary.get("publication_number") or source_summary.get("invention_title") or ""
        applicant = source_summary.get("primary_applicant") or ""
        current_todo = self.current_todo() or {}
        todo_title = str(current_todo.get("title") or current_todo.get("description") or "").strip()
        batch_id = str((self.active_run() or {}).get("active_batch_id") or "").strip()
        selected_document_count = self._startup_selected_document_count()

        if stage == "search-elements":
            parts = [f"我先{objective_clause}整理本轮检索要素。"]
            if source_label:
                parts.append(f"当前起点是 {source_label}。")
            if applicant:
                parts.append(f"我会顺带核对申请人边界，先确认是否需要锁定到 {applicant}。")
            else:
                parts.append("我会先把核心对象、关键效果和申请人/时间边界归并出来。")
            return " ".join(parts).strip()
        if stage == "plan-prober":
            return (
                f"我先对{objective_clause}的高风险检索点做低成本预检，"
                "优先检查术语是否过宽或过窄，以及布尔组合是否存在明显漏检风险。"
            )
        if stage == "planner":
            element_clause = f"当前已识别 {search_element_count} 个检索要素，" if search_element_count > 0 else ""
            return (
                f"我先基于现有上下文组织这轮检索计划。{element_clause}"
                "接下来会先定检索主线、数据库边界和后续验证点。"
            ).strip()
        if stage == "query-executor":
            todo_clause = f"当前步骤是“{todo_title}”。" if todo_title else "我先从当前检索步骤开始。"
            return (
                f"{todo_clause} 接下来会按既定数据库和检索蓝图拉起召回，"
                "先观察命中规模、去重情况和是否需要当轮调整。"
            ).strip()
        if stage == "coarse-screener":
            batch_clause = f"当前批次是 {batch_id}。" if batch_id else "我先载入当前候选批次。"
            return (
                f"{batch_clause} 接下来会按标题、摘要和分类线索做快速保留/淘汰，"
                "尽快把候选池压缩到值得精读的范围。"
            ).strip()
        if stage == "close-reader":
            batch_clause = f"当前精读批次是 {batch_id}。" if batch_id else "我先打开当前精读批次。"
            selected_clause = (
                f" 目前已进入候选池的文献约 {selected_document_count} 篇。"
                if selected_document_count > 0
                else ""
            )
            return (
                f"{batch_clause}{selected_clause} 接下来会逐篇核对关键证据、覆盖点和缺口，"
                "判断哪些文献可以继续进入对比。"
            ).strip()
        if stage == "feature-comparer":
            selected_clause = (
                f"当前可对比的文献约 {selected_document_count} 篇。"
                if selected_document_count > 0
                else "我先基于当前已选文献开展特征对比。"
            )
            return (
                f"{selected_clause} 接下来会核查关键特征的覆盖、部分覆盖和剩余缺口，"
                "并判断是否需要补检。"
            ).strip()
        return "我先同步当前阶段的目标和边界，然后继续执行后续检索动作。"

    def emit_startup_stage_log(self, *, stage_kind: str, runtime: Any | None = None) -> Optional[Dict[str, Any]]:
        descriptor = self._stage_descriptor(stage_kind)
        stage_instance_id = str(descriptor.get("stageInstanceId") or "").strip()
        if not stage_instance_id or self._has_stage_runtime_event(stage_instance_id, runtime_event="startup"):
            return None
        content = self._build_startup_stage_log_content(stage_kind)
        if not content:
            return None
        return self.write_stage_log(
            stage_kind=stage_kind,
            content=content,
            status="completed",
            append=False,
            extra_metadata={"logSource": "runtime", "runtimeEvent": "startup"},
            runtime=runtime,
        )
    ...
    def write_stage_log(
        ...
        *,
        ...
        descriptor = self._stage_descriptor(stage_kind)
        stage_instance_id = str(descriptor.get("stageInstanceId") or "").strip()
        delta = str(content or "").strip()
        next_content = delta
        normalized_status = str(status or "running").strip().lower() or "running"
        stream_status = "failed" if normalized_status == "failed" else "completed"
        next_metadata = {
            **descriptor,
        ...
        next_metadata["status"] = stream_status
        message_id = uuid.uuid4().hex
        self.storage.create_ai_search_message(
            {
                "message_id": message_id,
                "task_id": self.task_id,
                "plan_version": descriptor.get("planVersion"),
                "role": "assistant",
        ...
        if normalized_status == "completed":
            ...
            self.emit_stream_event(
                ...
                "assistant.stage.failed",
                {
                    ...
                    **descriptor,
                    "content": next_content,
                    "errorMessage": delta or "阶段执行失败",
                },
            )
        self.notify_snapshot_changed(runtime, reason="stage_log")
        return {
            "message_id": message_id,
    ...
    def _runtime_stage_checkpoint_content(self, stage_kind: str, checkpoint: str) -> str:
        normalized_stage = str(stage_kind or "").strip()
        normalized_checkpoint = str(checkpoint or "").strip()
        if normalized_checkpoint != "entered_execution":
            return ""
        content_map = {
            "search-elements": "开始进入检索要素整理，接下来会先归并目标、对象和非阻塞边界缺口。",
            "plan-prober": "开始进入规划预检，接下来会优先验证最可能导致漏检或噪声过大的高风险点。",
            "planner": "开始进入计划草案实际组装，接下来会先落展示正文，再补执行结构和子计划。",
            "query-executor": "开始进入当前检索步骤的实际执行，接下来会按既定蓝图发起检索并观察召回与去重情况。",
            "coarse-screener": "开始进入候选粗筛，接下来会按相关性快速保留或淘汰当前批次文献。",
            "close-reader": "开始进入重点精读，接下来会逐篇核查关键证据和仍未覆盖的技术点。",
            "feature-comparer": "开始进入特征对比，接下来会核查已覆盖、部分覆盖和仍待补强的关键特征。",
        }
        return str(content_map.get(normalized_stage) or "").strip()

    def emit_runtime_stage_checkpoint(
        self,
        *,
        stage_kind: str,
        checkpoint: str,
        ...
        if not stage_instance_id:
            return None
        normalized_checkpoint = str(checkpoint or "").strip()
        if not normalized_checkpoint or self._has_stage_runtime_event(
            stage_instance_id,
            runtime_checkpoint=normalized_checkpoint,
        ):
            return None
        content = self._runtime_stage_checkpoint_content(stage_kind, normalized_checkpoint)
        if not content:
            return None
        return self.write_stage_log(
            stage_kind=stage_kind,
            ...
            extra_metadata={
                ...
                "runtimeCheckpoint": normalized_checkpoint,
                "runtimeCheckpoints": [normalized_checkpoint],
            },
            runtime=runtime,
        )

    def _runtime_stage_tool_progress_content(
        self,
        *,
        stage_kind: str,
        tool_name: str,
        ...
        if normalized_stage != "planner":
            return ""
        if normalized_tool == "save_plan_review_markdown":
            return "已保存计划正文框架，接下来开始补执行结构和数据库边界。"
        if normalized_tool == "save_plan_execution_overview":
            return "已保存执行总览，接下来开始逐个写入子计划和检索步骤。"
        if normalized_tool == "append_plan_sub_plan":
            sub_plan_id = str(parsed_result.get("sub_plan_id") or "").strip()
            sub_plan_count = int(parsed_result.get("sub_plan_count") or 0)
            if sub_plan_id and sub_plan_count > 0:
                return f"已写入子计划 {sub_plan_id}，当前草案共包含 {sub_plan_count} 个子计划。"
            if sub_plan_id:
                return f"已写入子计划 {sub_plan_id}。"
            return "已写入一个子计划，接下来继续补齐剩余结构。"
        if normalized_tool == "finalize_plan_draft":
            return "已完成计划草案封口校验，接下来准备提交正式计划并进入确认。"
        return ""

    def emit_runtime_stage_tool_progress(
        self,
        *,
        stage_kind: str,
        tool_name: str,
        ...
        content = self._runtime_stage_tool_progress_content(
            stage_kind=stage_kind,
            tool_name=tool_name,
        ...
        return self.write_stage_log(
            ...
            extra_metadata={
                ...
                "runtimeToolName": str(tool_name or "").strip() or None,
            },
            runtime=runtime,
        )
