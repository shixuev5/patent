...
class FormalExaminer:
    ...
    def __init__(self, parts_db: Dict[str, Any], image_parts: Dict[str, List[str]]):
        """
        :param parts_db: 文本提取的部件库
        :param image_parts: 视觉识别的部件映射
        """
        ...
        for pid, info in raw_parts.items():
            ...
            if normalized in self.parts_db and self.parts_db[normalized] != info:
                logger.warning(f"部件标号归一化冲突：原始标号 {pid!r} 归一化为 {normalized!r}，将覆盖已有定义。")
    ...
    def _build_consistency_markdown(self, missing_in_images: List[str], undefined_in_text: List[str]) -> str:
        ...
        if missing_in_images:
            lines.append(f"**1. 说明书文字部分存在，但附图中未标记 ({len(missing_in_images)}处)：**")
            for idx, pid in enumerate(missing_in_images):
                if idx >= 20:
                    lines.append(f"- ... (共 {len(missing_in_images)} 条，仅显示前 20 条)")
                    break
                part_info = self.parts_db.get(pid, {}) if isinstance(self.parts_db, dict) else {}
        ...
        if undefined_in_text:
            if missing_in_images:
                lines.append("")
            lines.append(f"**2. 附图存在标记，但说明书文字部分未定义 ({len(undefined_in_text)}处)：**")
        ...
        lines.append("\n> *注：以上结果基于 OCR 与大模型多模态解析自动生成，请人工核查确认。*")
        return "\n".join(lines)