...
入口文件：`patent_agents/ai_review/main.py`

---

## 2. 工作流拓扑

`create_workflow()`（`patent_agents/ai_review/main.py`）定义如下：
...
这 5 个节点在 `patent_agents/ai_review/src/nodes/` 下独立实现，行为与主分析流水线保持一致：
...
- 节点实现位于 `patent_agents/ai_review/src/nodes/check_node.py`。
...
实现说明：
- `ai_review` 运行时不依赖 `patent_agents/patent_analysis` 包内节点定义。
...
```bash
python -m patent_agents.ai_review.main --pn CN202010000000.0 --task-id review001
```

按上传文件：

```bash
python -m patent_agents.ai_review.main --upload-file /path/to/patent.pdf --task-id review002
```
