...
from patent_agents.ai_reply.src.state import ReviewUnit
from patent_agents.ai_reply.src.utils import PipelineCancelled, ensure_not_cancelled, get_node_cache
...
4. 结构与句式继承原则：如果 `review_before_text` 已形成稳定的段落骨架、编号骨架或句式模板（例如“区别特征总述 -> 对于区别特征① -> 对于区别特征② -> 对于区别特征③ -> 结论”），`review_text` 必须保留同样的组织方式、段落顺序和句式节奏。合并进来的内容或补充论证必须嵌回对应原段内部，不得把后段内容前置，不得另起一套平行论述结构。
5. 对应段内补充原则：若新增限定或补充论证明显属于原文某一编号段（例如原文“对于区别特征③”段），则必须写入该段内部，并沿用该段原有开头方式和说理节奏；不允许在“对于区别特征①/②”之后插入新的总括段，再在末尾补写“对于区别特征③”。
6. 结论绝对对齐：只要 assessment_reasoning/verdict 素材中提及“克服了缺陷”或“具备创造性”，你的最终评述结论必须与其完全一致；如果结论是“未克服”，则必须基于素材清晰阐述驳回逻辑。
7. 纯净输出：整合成一段连贯流畅的专业文本，只输出评述正文。不得输出单元标题，不得带有“审查员认为”等非正式的口语化开场白。

【针对不同单元类型的处理约束】
- type = "evidence_restructured" (独权主卡重组)：
  代表某个独权体系的主评述。你必须以 oa_materials 和 review_before_text 为骨架，将原独权评述、并入的从权评述以及答复/修改评估素材自然融合。**必须保留原OA中已存在的详细论证过程，严禁只写结论省略分析。** 若 `review_before_text` 已有“对于区别特征①/②/③”之类分段，新增素材只能嵌入对应分段内部，不能打散原骨架。
...
- type = "supplemented_new" (新增或修改特征补充)：
  原OA中无该部分评述。你必须完全依赖 amendment_materials 或 response_materials 中的审查员评估结论撰写。清晰指出新增/修改特征是什么，并阐明其为何能够/不能够克服原缺陷。若存在可复用的上一轮相邻句式或同类从权模板，应尽量沿用同句式展开，不要自由改写成另一种风格。
  ...
  "review_text": "最终的评述正文。直接是一段完整的话，不加任何标题。请确保文本内的双引号和换行符已正确转义。"
    ...
    def _build_single_user_prompt(self, unit_input: Dict[str, Any]) -> str:
        return (
            ...
            "【结构要求】若 review_before_text 已按固定句式、段落顺序或“对于区别特征①/②/③”编号骨架组织，review_text 必须保持同样结构；补充内容只能写回对应原段内部，不得另起总括段或打乱顺序。\n"
            "=== 待处理的评述单元素材 ===\n"
