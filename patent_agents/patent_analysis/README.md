...
入口文件：`patent_agents/patent_analysis/main.py`

---

## 2. 工作流拓扑

`create_workflow()`（`patent_agents/patent_analysis/main.py`）当前挂载的节点如下：
...
- AI 审查链路已拆分到 `patent_agents/ai_review`，本工作流仅负责 AI 分析与检索策略产出。
...
```bash
python -m patent_agents.patent_analysis.main --pn CN202010000000.0 --task-id demo001
```

按上传 PDF 运行：

```bash
python -m patent_agents.patent_analysis.main --upload-file /path/to/patent.pdf --task-id demo002
```
