  ...
  >
    ...
    <div v-show="isExpanded" class="panel-content custom-scrollbar">
      <div v-if="taskStore.tasks.length === 0" class="empty-state">
        ...
        <p class="text-sm text-gray-400 mt-1">输入专利号或上传PDF文件开始分析</p>
      </div>
          ...
          <div class="modal-content" @click.stop>
            ...
            <div class="modal-body">
              ...
              <div v-if="taskStore.groupedTasks.processing.length > 0" class="warning-box mt-4">
                ...
                <p>注意：有 <strong>{{ taskStore.groupedTasks.processing.length }}</strong> 个任务正在处理中，清空将中断这些任务。</p>
              </div>
            </div>
...
const downloadAll = () => {
  ...
  completedTasks.forEach((task, index) => {
    setTimeout(() => {
      taskStore.downloadResult(task)
    }, index * 500)
  })
}

// Auto-expand if there are processing tasks
onMounted(() => {
  if (taskStore.hasProcessingTasks) {
