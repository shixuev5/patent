<template>
  <div class="min-h-screen bg-gradient-to-b from-slate-50 via-slate-50 to-blue-50">
    ...
    <main class="mx-auto max-w-5xl px-4 py-10 sm:py-14">
      <section class="mb-8 text-center">
        <h1 class="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">专利智能处理平台</h1>
        <p class="mt-3 text-sm leading-6 text-slate-600 sm:text-base">
          支持专利分析与审查意见答复两类任务，统一任务队列与报告下载。
        </p>
      </section>

      <section class="rounded-3xl border border-slate-200/80 bg-white/90 p-5 shadow-xl shadow-slate-200/40 backdrop-blur sm:p-7">
        ...
        <div v-if="mode === 'patent_analysis'" class="mt-6 space-y-5">
          <div>
          ...
          <button
            ...
            class="w-full rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300"
            @click="submitPatentTask"
          >
            <span v-if="loading">提交中...</span>
            <span v-else>创建专利分析任务</span>
          </button>
        </div>
        ...
        <div v-else class="mt-6 space-y-5">
          ...
          >
            <span v-if="loading">提交中...</span>
            <span v-else>创建审查意见答复任务</span>
          </button>
        </div>
      </section>

      <section class="mt-6 flex flex-wrap items-center gap-3 text-xs text-slate-500">
        <span class="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-emerald-700">服务正常</span>
        <span class="rounded-full border border-slate-200 bg-white px-3 py-1">已分析任务：{{ analyzedPatentCountText }}</span>
        <span v-if="serviceVersion" class="rounded-full border border-slate-200 bg-white px-3 py-1">版本：v{{ serviceVersion }}</span>
      </section>
    </main>
...
const onPatentFileChange = (event: Event) => {
  ...
  if (!selected.name.toLowerCase().endsWith('.pdf')) {
    taskStore.showGlobalNotice('error', '专利分析仅支持 PDF 文件。')
    target.value = ''
    return
  }
...
const submitOfficeActionTask = async () => {
  ...
  try {
    ...
    if (result.ok) {
      ...
      taskStore.showGlobalNotice('success', result.message || '任务已创建，正在处理。')
    } else {
      taskStore.showGlobalNotice('error', result.error || '任务创建失败，请重试。')
    }
  } finally {
