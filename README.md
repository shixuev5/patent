...
3. **配置环境变量**
   ...
   # 检索网关配置（embedding + rerank，可选；不填则回退 LLM_API_KEY / LLM_BASE_URL）
   RETRIEVAL_API_KEY=your_dashscope_api_key
   RETRIEVAL_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1
   RETRIEVAL_EMBEDDING_MODEL=text-embedding-v4
   RETRIEVAL_RERANK_MODEL=qwen3-rerank
   ...
   # 模型价格同步配置（自动从百炼官方价格页抓取中国内地按量调用价格）
   TOKEN_PRICING_SYNC_TTL_SECONDS=604800
   ...
   ACADEMIC_RETRIEVAL_CACHE_TTL_SECONDS=21600

   # 智慧芽配置（多账号）
   ZHIHUIYA_ACCOUNTS__0__USERNAME=your_zhihuiya_username_1
   ZHIHUIYA_ACCOUNTS__0__PASSWORD=your_zhihuiya_password_1
   ZHIHUIYA_ACCOUNTS__1__USERNAME=your_zhihuiya_username_2
   ZHIHUIYA_ACCOUNTS__1__PASSWORD=your_zhihuiya_password_2
   ...
   # 微信 IM 接入（按需开启）
   WECHAT_INTEGRATION_ENABLED=false
   INTERNAL_GATEWAY_TOKEN=replace-with-a-random-shared-token
   API_BASE_URL=http://127.0.0.1:7860
   IM_GATEWAY_POLL_INTERVAL_SECONDS=8
   IM_GATEWAY_DOWNLOAD_DIR=/app/data/im_gateway
   IM_GATEWAY_CRED_R2_PREFIX=im-gateway/credentials
   IM_GATEWAY_CRED_ENCRYPTION_KEY=replace-with-a-long-random-secret
   ```
...
- **微信登录态持久化**：如启用 `im-gateway`，建议同时配置 `R2_ENABLED=true`、`IM_GATEWAY_CRED_R2_PREFIX` 和 `IM_GATEWAY_CRED_ENCRYPTION_KEY`，将加密后的微信 SDK 凭证文件持久化到 R2，避免每次部署重新扫码
- **端口配置**：Hugging Face Spaces 默认使用端口 7860
- **访问控制**：如果需要设置访问密码，可以使用 Hugging Face Spaces 的访问控制功能
- **微信网关**：当前 Docker Space 会始终启动 `backend`，并在 `WECHAT_INTEGRATION_ENABLED=true` 时于同一容器内以 `python -m im_gateway.main` 方式启动微信网关
