...
3. **配置环境变量**
   ...
   IM_GATEWAY_CRED_R2_KEY=secure/im-gateway/credentials.enc
...
- **微信登录态持久化**：如启用 `im-gateway`，建议同时配置 `R2_ENABLED=true`、`IM_GATEWAY_CRED_R2_KEY` 和 `IM_GATEWAY_CRED_ENCRYPTION_KEY`，将加密后的微信 SDK 凭证文件持久化到 R2，避免每次部署重新扫码
