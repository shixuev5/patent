...
def setup_node(state: AgentState) -> Dict[str, Any]:
    ...
    return {
        ...
        "current_phase": PHASE_TIER0_SCOUT,
        "iteration_count": 0,
        "executed_queries": [],
...
def reviewer_node(state: AgentState) -> Dict[str, Any]:
    ...
    # 优先看 Rerank 分数高的
    unreviewed_docs = [
        d for d in docs if (d.get("uid") or d.get("id")) not in reviewed_uids
    ]
    # 按 Rerank Score 排序
    ...
    logger.info(
        f"Reviewing {len(candidates)} new candidates (Total unreviewed: {len(unreviewed_docs)})..."
    )
    ...
    for d in candidates:
        ...
        if uid:
            new_reviewed_uids.add(uid)

    # 4. [P0-3] 学习机制 (防止污染)
    # 仅使用被 QuickEye 认为有价值的文档 (match_score > 20 或 best_doc)
    high_value_docs = [
        d for d in candidates if d.get("analysis", {}).get("match_score", 0) > 20
    ]
    ...
    if high_value_docs:
        ...
        updated_matrix = hand.harvest_new_keywords(
            docs=high_value_docs, current_matrix=state["search_matrix"]
        )
    ...
    if current_phase == PHASE_TIER2_Y and has_d1 and missing_feats:
        ...
        for cand in candidates:
            ...
            if check_res.get("is_disclosed"):
                logger.success(
                    f">>> 拼图成功！D1({d1_doc['uid']}) + D2({cand['uid']}) 覆盖特征 [{target_feat}]"
                )
                best_combination = {
                    "d1": d1_doc,
                    "d2": cand,
    ...
    if current_phase == PHASE_TIER0_SCOUT:
        # 侦察结束，无论有没有结果，都进入 Tier 1 精准打击
        # Tier 1 将受益于 Tier 0 产生的 validated_ipcs 和 updated_matrix
        logger.info(">>> 侦察阶段结束。进入 Tier 1 精准打击...")
        next_phase = PHASE_TIER1_X

    elif current_phase == PHASE_TIER1_X:
        if has_d1 and missing_feats:
            next_phase = PHASE_TIER2_Y  # 进 Tier 2 找 D2
        elif (
            not has_d1
            and len(docs) > 0
            and state["iteration_count"] < state["max_iterations"] - 1
        ):
            # [FIX] 如果没找到 D1，但本轮有命中(可能Matrix进化了)，再试一次 Tier 1
            logger.info(
                "Tier 1 未命中强相关 D1，但 Matrix 已更新，保持 Tier 1 进行精炼..."
            )
            next_phase = PHASE_TIER1_X
        else:
...
def reporter_node(state: AgentState) -> Dict[str, Any]:
    ...
    logger.success(
        f"Report Generated. Docs: {metrics['total_hits']}, Queries: {metrics['total_queries']}"
    )

    if state.get("best_evidence") and state["best_evidence"].get("is_x_class"):
        logger.success(
            f"Final Outcome: [X] Novelty Destroyed by {state['best_evidence'].get('pn')}"
        )
    elif state.get("best_combination"):
        logger.success(
            f"Final Outcome: [Y] Combination Found ({state['best_combination']['d1']['pn']} + {state['best_combination']['d2']['pn']})"
        )
    else:
