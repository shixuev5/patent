...
def test_rejection_drafting_ignores_amendment_review_disputes(monkeypatch) -> None:
    node = RejectionDraftingNode()

    def _fake_invoke_text_json(messages, task_kind, temperature):
        assert '"dispute_id": "DSP_RESPONSE"' in messages[1]["content"]
        assert '"dispute_id": "TOPUP_F1"' not in messages[1]["content"]
        return {
            "items": [
                {
                    "dispute_id": "DSP_RESPONSE",
                    "final_examiner_rejection_reason": "经审查，权利要求1仍不具备创造性。",
                }
            ]
        }
