...
class D1TaskStorage:
    ...
    def _row_to_ai_search_plan(self, row: Dict[str, Any]) -> Dict[str, Any]:
        return {
            ...
            "search_elements_json": self._parse_metadata(row.get("search_elements_json")),
            "plan_json": self._parse_metadata(row.get("plan_json")),
            "created_at": row.get("created_at"),
        ...
        for record in records:
            ...
            result = self._request(
                ...
                INSERT INTO ai_search_documents (
                    ...
                    ipc_cpc_json, source_batches_json, stage, score, agent_reason,
                    key_passages_json, user_pinned, user_removed, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def update_ai_search_document(self, task_id: str, plan_version: int, document_id: str, **kwargs) -> bool:
        ...
        result = self._request(
            ...
            """,
            [*updates.values(), task_id, int(plan_version), document_id],
        )
        return self._changed_rows(result) > 0

    def _row_to_ai_search_feature_table(self, row: Dict[str, Any]) -> Dict[str, Any]:
        return {
            "feature_table_id": row.get("feature_table_id"),
            "task_id": row.get("task_id"),
            "plan_version": int(row["plan_version"]),
            "status": row.get("status"),
            "table_json": self._parse_metadata(row.get("table_json")),
    ...
    def get_ai_search_feature_table(
        ...
        if feature_table_id:
            row = self._fetchone(
                ...
                LIMIT 1
                """,
                [task_id, int(plan_version), feature_table_id],
            )
        else:
            row = self._fetchone(
                """
