...
class SQLiteTaskStorage:
    ...
    def _row_to_wechat_binding(self, row: sqlite3.Row) -> WeChatBinding:
        return WeChatBinding(
            binding_id=str(row["binding_id"]),
            owner_id=str(row["owner_id"]),
            status=str(row["status"]),
            bot_account_id=row["bot_account_id"] if "bot_account_id" in row.keys() else None,
            wechat_peer_id=row["wechat_peer_id"] if "wechat_peer_id" in row.keys() else None,
            wechat_peer_name=row["wechat_peer_name"] if "wechat_peer_name" in row.keys() else None,
            push_task_completed=bool(row["push_task_completed"]) if "push_task_completed" in row.keys() else True,
            push_task_failed=bool(row["push_task_failed"]) if "push_task_failed" in row.keys() else True,
            push_ai_search_pending_action=bool(row["push_ai_search_pending_action"]) if "push_ai_search_pending_action" in row.keys() else True,
            bound_at=parse_storage_ts(row["bound_at"], naive_strategy="utc") if row["bound_at"] else None,
            disconnected_at=parse_storage_ts(row["disconnected_at"], naive_strategy="utc") if row["disconnected_at"] else None,
            last_inbound_at=parse_storage_ts(row["last_inbound_at"], naive_strategy="utc") if row["last_inbound_at"] else None,
            last_outbound_at=parse_storage_ts(row["last_outbound_at"], naive_strategy="utc") if row["last_outbound_at"] else None,
            created_at=parse_storage_ts(row["created_at"], naive_strategy="utc"),
            updated_at=parse_storage_ts(row["updated_at"], naive_strategy="utc"),
        )

    def _row_to_wechat_bind_session(self, row: sqlite3.Row) -> WeChatBindSession:
        return WeChatBindSession(
            bind_session_id=str(row["bind_session_id"]),
            owner_id=str(row["owner_id"]),
            status=str(row["status"]),
            bind_code=str(row["bind_code"]),
    ...
    def _row_to_wechat_flow_session(self, row: sqlite3.Row) -> WeChatFlowSession:
        return WeChatFlowSession(
            ...
            draft_payload=self._parse_metadata(row["draft_payload_json"] if "draft_payload_json" in row.keys() else None),
            expires_at=parse_storage_ts(row["expires_at"], naive_strategy="utc") if row["expires_at"] else None,
            created_at=parse_storage_ts(row["created_at"], naive_strategy="utc"),
            updated_at=parse_storage_ts(row["updated_at"], naive_strategy="utc"),
        )

    def _row_to_wechat_delivery_job(self, row: sqlite3.Row) -> WeChatDeliveryJob:
        return WeChatDeliveryJob(
            delivery_job_id=str(row["delivery_job_id"]),
            owner_id=str(row["owner_id"]),
            binding_id=row["binding_id"] if "binding_id" in row.keys() else None,
            task_id=row["task_id"] if "task_id" in row.keys() else None,
            event_type=str(row["event_type"]),
            status=str(row["status"]),
            payload=self._parse_metadata(row["payload_json"] if "payload_json" in row.keys() else None),
            ...
            next_attempt_at=parse_storage_ts(row["next_attempt_at"], naive_strategy="utc") if row["next_attempt_at"] else None,
            claimed_at=parse_storage_ts(row["claimed_at"], naive_strategy="utc") if row["claimed_at"] else None,
            completed_at=parse_storage_ts(row["completed_at"], naive_strategy="utc") if row["completed_at"] else None,
            failed_at=parse_storage_ts(row["failed_at"], naive_strategy="utc") if row["failed_at"] else None,
        ...
        with self._get_connection() as conn:
            conn.execute(
                ...
                (
                    ...
                    session.qr_svg,
                    to_utc_z(session.expires_at, naive_strategy="utc"),
                    session.bot_account_id,
                    session.wechat_peer_id,
                    session.wechat_peer_name,
                    session.error_message,
                    to_utc_z(session.bound_at, naive_strategy="utc") if session.bound_at else None,
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM wechat_bind_sessions WHERE bind_session_id = ?",
                (session.bind_session_id,),
            ).fetchone()
        if row is None:
    ...
    def get_wechat_bind_session_by_code(self, bind_code: str) -> Optional[WeChatBindSession]:
        ...
        with self._get_connection() as conn:
            row = conn.execute(
                """
                SELECT * FROM wechat_bind_sessions
    ...
    def get_current_wechat_bind_session(self, owner_id: str) -> Optional[WeChatBindSession]:
        with self._get_connection() as conn:
            row = conn.execute(
                """
                SELECT * FROM wechat_bind_sessions
                WHERE owner_id = ? AND status IN ('pending', 'scanned', 'bound')
                ORDER BY created_at DESC
                LIMIT 1
                """,
                (owner_id,),
    ...
    def update_wechat_bind_session(self, bind_session_id: str, **updates: Any) -> Optional[WeChatBindSession]:
        ...
        for key, value in normalized.items():
            if key in {"created_at", "updated_at", "expires_at", "bound_at"} and value is not None:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                f"UPDATE wechat_bind_sessions SET {assignments} WHERE bind_session_id = ?",
                tuple(values),
            )
            conn.commit()
            if cursor.rowcount <= 0:
    ...
    def get_wechat_binding_by_peer(self, bot_account_id: str, wechat_peer_id: str) -> Optional[WeChatBinding]:
        with self._get_connection() as conn:
            row = conn.execute(
                """
                SELECT * FROM wechat_bindings
                WHERE bot_account_id = ? AND wechat_peer_id = ? AND status = 'active'
                ORDER BY updated_at DESC
                LIMIT 1
                """,
    ...
    def upsert_wechat_binding(self, binding: WeChatBinding) -> WeChatBinding:
        ...
        with self._get_connection() as conn:
            conn.execute(
                ...
                WHERE owner_id = ? AND status = 'active'
                """,
                (now_iso, now_iso, binding.owner_id),
            )
            ...
            conn.execute(
                ...
                ON CONFLICT(binding_id) DO UPDATE SET
                    ...
                    wechat_peer_id = excluded.wechat_peer_id,
                    wechat_peer_name = excluded.wechat_peer_name,
                    push_task_completed = excluded.push_task_completed,
                    push_task_failed = excluded.push_task_failed,
                    push_ai_search_pending_action = excluded.push_ai_search_pending_action,
                ...
                (
                    ...
                    binding.wechat_peer_name,
                    1 if binding.push_task_completed else 0,
                    1 if binding.push_task_failed else 0,
                    1 if binding.push_ai_search_pending_action else 0,
                    ...
                    to_utc_z(binding.created_at, naive_strategy="utc"),
                    to_utc_z(binding.updated_at, naive_strategy="utc"),
                ),
            )
            conn.commit()
            row = conn.execute(
                "SELECT * FROM wechat_bindings WHERE binding_id = ?",
                (binding.binding_id,),
            ).fetchone()
        if row is None:
    ...
    def update_wechat_binding(self, binding_id: str, **updates: Any) -> Optional[WeChatBinding]:
        ...
        values = []
        for key, value in normalized.items():
            if key in {"bound_at", "disconnected_at", "last_inbound_at", "last_outbound_at", "created_at", "updated_at"} and value is not None:
                values.append(to_utc_z(value, naive_strategy="utc"))
            else:
                values.append(value)
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                f"UPDATE wechat_bindings SET {assignments} WHERE binding_id = ?",
                tuple(values),
            )
            conn.commit()
            if cursor.rowcount <= 0:
    ...
    def disconnect_wechat_binding(self, owner_id: str) -> Optional[WeChatBinding]:
        ...
        if not binding:
            return None
        now_iso = utc_now_z()
        return self.update_wechat_binding(
            binding.binding_id,
            status="disconnected",
        ...
        with self._get_connection() as conn:
            conn.execute(
                ...
                (
                    ...
                    flow_session.owner_id,
                    flow_session.flow_type,
                    flow_session.status,
                    flow_session.current_step,
                    ...
                    to_utc_z(flow_session.created_at, naive_strategy="utc"),
                    to_utc_z(flow_session.updated_at, naive_strategy="utc"),
                ),
            )
            conn.commit()
            row = conn.execute(
                "SELECT * FROM wechat_flow_sessions WHERE flow_session_id = ?",
                (flow_session.flow_session_id,),
            ).fetchone()
        if row is None:
    ...
    def get_active_wechat_flow_session(self, owner_id: str, flow_type: str) -> Optional[WeChatFlowSession]:
        with self._get_connection() as conn:
            row = conn.execute(
                """
                SELECT * FROM wechat_flow_sessions
                WHERE owner_id = ? AND flow_type = ? AND status = 'active'
                ORDER BY updated_at DESC
                LIMIT 1
                """,
                (owner_id, flow_type),
    ...
    def upsert_wechat_flow_session(
        self,
        owner_id: str,
        flow_type: str,
        *,
        ...
        with self._get_connection() as conn:
            conn.execute(
                ...
                (
                    ...
                    status,
                    current_step,
                    self._encode_json_value(draft_payload),
                    to_utc_z(expires_at, naive_strategy="utc") if expires_at else None,
                    existing.created_at.isoformat() if existing else now_iso,
                    now_iso,
                ),
            )
            conn.commit()
    ...
    def resolve_wechat_flow_session(self, owner_id: str, flow_type: str, status: str = "completed") -> bool:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                """
                UPDATE wechat_flow_sessions
                SET status = ?, updated_at = ?
                WHERE owner_id = ? AND flow_type = ? AND status = 'active'
                """,
                (status, now_iso, owner_id, flow_type),
            )
            conn.commit()
        return cursor.rowcount > 0
        ...
        with self._get_connection() as conn:
            conn.execute(
                ...
                (
                    ...
                    job.last_error,
                    to_utc_z(job.created_at, naive_strategy="utc"),
                    to_utc_z(job.updated_at, naive_strategy="utc"),
                ),
            )
            conn.commit()
            row = conn.execute(
                "SELECT * FROM wechat_delivery_jobs WHERE delivery_job_id = ?",
                (job.delivery_job_id,),
            ).fetchone()
        if row is None:
    ...
    def claim_wechat_delivery_jobs(self, limit: int = 1) -> List[WeChatDeliveryJob]:
        ...
        with self._get_connection() as conn:
            rows = conn.execute(
                """
                SELECT * FROM wechat_delivery_jobs
            ...
            claimed_ids = [str(row["delivery_job_id"]) for row in rows]
            if claimed_ids:
                placeholders = ", ".join("?" for _ in claimed_ids)
                conn.execute(
                    f"""
                    UPDATE wechat_delivery_jobs
                    SET status = 'processing', claimed_at = ?, updated_at = ?, attempt_count = attempt_count + 1
                    WHERE delivery_job_id IN ({placeholders})
                    """,
                    (now_iso, now_iso, *claimed_ids),
                )
                conn.commit()
                rows = conn.execute(
                    f"SELECT * FROM wechat_delivery_jobs WHERE delivery_job_id IN ({placeholders})",
                    tuple(claimed_ids),
                ).fetchall()
        return [self._row_to_wechat_delivery_job(row) for row in rows]
    ...
    def update_wechat_delivery_job(self, delivery_job_id: str, **updates: Any) -> Optional[WeChatDeliveryJob]:
        ...
        for key, value in normalized.items():
            ...
            elif key in {"next_attempt_at", "claimed_at", "completed_at", "failed_at", "created_at", "updated_at"} and value is not None:
                values.append(to_utc_z(value, naive_strategy="utc"))
            elif key == "payload":
                values.append(self._encode_json_value(value))
            else:
                values.append(value)
        # support callers using payload instead of payload_json
        assignments = assignments.replace("payload = ?", "payload_json = ?")
        values.append(str(delivery_job_id or "").strip())
        with self._get_connection() as conn:
            cursor = conn.execute(
                f"UPDATE wechat_delivery_jobs SET {assignments} WHERE delivery_job_id = ?",
                tuple(values),
            )
            conn.commit()
            if cursor.rowcount <= 0:
    ...
    def _row_to_ai_search_document(self, row: sqlite3.Row) -> Dict[str, Any]:
        return {
            ...
            "detail_source": row["detail_source"] if "detail_source" in row.keys() else None,
            "created_at": row["created_at"],
            "updated_at": row["updated_at"],
        }
        ...
        with self._get_connection() as conn:
            cursor = conn.executemany(
                ...
                INSERT INTO ai_search_documents (
                    ...
                    detail_fingerprint, detail_source, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
